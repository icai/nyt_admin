<?php
/**
 * [瑞恩授权系统 System] Copyright (c) 2018 WEBY.CC
 * 瑞恩授权系统 is NOT a free software, it under the license terms, visited http://www.weby.cc/ for more details.
 */
defined('IN_IA') or exit('Access Denied');
define('HTTP_X_FOR', (isset($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] == '443') ? 'https://' : 'http://');
define('CLOUD_GATEWAY_URL', HTTP_X_FOR .'we7.rewlkj.com/app/api.php');
define('CLOUD_GATEWAY_URL_NORMAL', HTTP_X_FOR .'we7.rewlkj.com/app/api.php');
...
function cloud_build() {
    ...
    $dat = cloud_request(HTTP_X_FOR .'we7.rewlkj.com/app/api.php', $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request(HTTP_X_FOR .'we7.rewlkj.com/app/api.php', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
    ...
    $response = cloud_request(HTTP_X_FOR .'we7.rewlkj.com/app/api.php', $pars);
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request(HTTP_X_FOR .'we7.rewlkj.com/app/api.php', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = HTTP_X_FOR .'we7.rewlkj.com/api/auth.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request(HTTP_X_FOR .'we7.rewlkj.com/app/api.php', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
