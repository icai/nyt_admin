...
define('TM_GLOBONUS_PAY', 'TM_GLOBONUS_PAY'); //分红打款
define('TM_GLOBONUS_UPGRADE', 'TM_GLOBONUS_UPGRADE'); //升级
define('TM_GLOBONUS_BECOME', 'TM_GLOBONUS_BECOME'); //成为股东通知
...
if (!class_exists('GlobonusModel')) {
    ...
    class GlobonusModel extends PluginModel
    {
        ...
        public function getSet($uniacid = 0)
        {
            ...
            $set['texts'] = array(
                'partner' => empty($set['texts']['partner']) ? '股东' : $set['texts']['partner'],
                'center' => empty($set['texts']['center']) ? '股东中心' : $set['texts']['center'],
                'become' => empty($set['texts']['become']) ? '成为股东' : $set['texts']['become'],
        ...
        function getLevels($all = true, $default = false)
        {
            ...
            if ($default) {
                $default = array(
                    'id' => '0',
                    "levelname" => empty($_S['globonus']['levelname']) ? '默认等级' : $_S['globonus']['levelname'],
                    'bonus' => $_W['shopset']['globonus']['bonus']
                );
        ...
        function getBonus($openid = '',$params = array())
        {
            ...
            if(in_array('ok',$params)) {
                $ret['ok']= pdo_fetchcolumn('select ifnull(sum(paymoney),0) from ' . tablename('ewei_shop_globonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
            ...
            if(in_array('lock',$params)) {
                ...
                if(!empty($billdData)){
                    ...
                    $id = implode(',',$ids);
                    $ret['lock']= pdo_fetchcolumn('select ifnull(sum(paymoney),0) from ' . tablename('ewei_shop_globonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid  and billid in('.$id.') limit 1',
                        array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                }
            }
        ...
        function sendMessage($openid = '', $data = array(), $message_type = '')
        {
            ...
            if ($message_type == TM_GLOBONUS_PAY && empty($usernotice['globonus_pay'])) {
                ...
                if($tm['is_advanced']){
                    ...
                    //股东升级  [昵称] [旧等级] [新等级] [旧等级分红比例] [新等级] [新等级分红比例] [时间]
                    $message = $tm['upgrade'];
        ...
        function upgradeLevelByOrder($openid)
        {
            ...
            if ($leveltype == 4 || $leveltype == 5) { //自购升级
                ...
                if (empty($oldlevel['id'])) {
                    $oldlevel = array(
                        'levelname' => empty($set['levelname']) ? '普通股东' : $set['levelname'],
                        'bonus' => $set['bonus']
                    );
                }
        ...
        function upgradeLevelByAgent($openid)
        {
            ...
            //6下级总人数（股东+非股东）7 一级下级人数（股东+非股东）
            //8团队总人数（股东）9一级团队人数（股东）
            //10下线总人数（非股东） 11一级下线人数（非股东）
        ...
        function getBonusData($year = 0, $month = 0, $week = 0,$openid = '')
        {
            ...
            $partners = pdo_fetchall('select m.id,m.openid,m.partnerlevel,l.bonus from ' . tablename('ewei_shop_member') . ' m '
                . '  left join ' . tablename('ewei_shop_globonus_level') . ' l on l.id = m.partnerlevel '
                . "  where m.uniacid=:uniacid and  m.ispartner=1 and m.partnerstatus=1 {$pcondition}", array(':uniacid' => $_W['uniacid']));
            //股东分红比例
            ...
            foreach ($partners as &$p) {
                ...
                $p['charge']=0;
                $p['chargemoney']=0;
                if (floatval($set['paycharge']) <= 0 || ($p['bonusmoney'] >= floatval($set['paybegin']) && $p['bonusmoney'] <= floatval($set['payend']))) {
                    //免收税
        ...
        function getTotals(){
            global $_W;
            return array(
                'total0'=>pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_globonus_bill').' where uniacid=:uniacid and status=0 limit 1',array(':uniacid'=>$_W['uniacid'])),
                'total1'=>pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_globonus_bill').' where uniacid=:uniacid and status=1 limit 1',array(':uniacid'=>$_W['uniacid'])),
                'total2'=>pdo_fetchcolumn('select count(*) from '.tablename('ewei_shop_globonus_bill').' where uniacid=:uniacid and status=2  limit 1',array(':uniacid'=>$_W['uniacid']))
            );
        }
    }
