    ...
    function getesheetTemp(){
        ...
        if($_W['ispost']){
            ...
            if(empty($_GPC['printarr']['ordercode']) && empty($template) ){
                ...
                $commodityOne["GoodsName"] = mb_substr($goodsname,0,15);//商品名称
                ...
                if($result["ResultCode"] == "100" || $result['Success'] === true) {
                    ...
                    die(json_encode(array("result"=>'success','PrintTemplate'=>$result['PrintTemplate'],'ordercode'=>$eorder["OrderCode"],'issend'=>$esheet_item['issend'])));
                }
                else {
                ...
                if(!empty($template)) {
                    //修改打印次数
                    pdo_query("UPDATE " . tablename('ewei_shop_order_goods') . " SET `esheetprintnum` = `esheetprintnum` + 1 WHERE id in (" . $_GPC['printarr']['ordergoodid'] . ")");
                    die(json_encode(array("result" => 'success', 'PrintTemplate' => $template, 'issend' => $esheet_item['issend'])));
                }else{
                    die(json_encode(array("result"=>'error','resp'=>'错误，请重试')));
                }
            }
        }
    }
