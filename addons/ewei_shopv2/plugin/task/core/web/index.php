...
class Index_EweiShopV2Page extends PluginWebPage {
    ...
    public function post() {
        ...
        if ($_W['ispost']) {
            ...
            //奖励数据 1：积分2：会员余额3：红包4：指定价格的商品5：优惠券
            $reward = array();
            ...
            if(!empty($sub_reward)){
                foreach($sub_reward as $val){
                    if($val['type'] == 1){
                        ...
                        $sub_data['money']['type']=intval($val['moneytype']);//0:余额1：微信
                    }elseif($val['type']==3){
    ...
    public function delreward(){
        global $_W, $_GPC;
        $id = intval($_GPC['id']);
        $type = intval($_GPC['del_type']);//1rec 2sub
        $datatype = intval($_GPC['data_type']);//1:credit 2:money 3:bribery 4:goods 5:coupon
...
//            pdo_update('ewei_shop_task_type',array('verb'=>$diy['verb'],'unit'=>$diy['unit']),array('type_key'=>'poster'));
            ...
            $posttype = trim($_GPC['posttype']);//保存类型,各tab页分别保存
            //任务标题
            ...
            $taskType = $this->model->getTaskType($data['type']);//得到类型详情
            ...
            if ($taskType['type_key'] == 'goods') {
                ...
                $data['design_data'] = $data['requiregoods'];//此字段在此任务类型时临时存储指定商品原始数据
            }
...
//          二级奖励
            ...
            if ($data['type'] == 'poster') {
                $data['design_data'] = htmlspecialchars_decode($_GPC['design_data']);//海报数据
                $data['keyword_pick'] = trim($_GPC['keyword_pick']);//关键词触发海报
                if (!empty($_GPC['member_group'])) {
                    $data['member_group'] = implode(',', $_GPC['member_group']);//会员指定分组
                } else {
                    $data['member_group'] = '';
                }
                if (!empty($_GPC['member_level'])) {
                    $data['member_level'] = intval($_GPC['member_level']);//会员指定等级
                } else {
                    $data['member_level'] = 0;
                }
                $data['auto_pick'] = intval($_GPC['auto_pick']);//自动接取海报任务
            }
            ...
            if(empty($task['verb'])){
                $task['verb'] = $poster_text['verb'];
            }
            if(empty($task['unit'])){
                $task['unit'] = $poster_text['unit'];
            }*/
