...
class LiveModel extends PluginModel
{
    ...
    public function isLiving($roomid=0) {
        ...
        if(!empty($roomid)){
            $live = pdo_fetch('SELECT id FROM '. tablename('ewei_shop_live'). ' WHERE id=:id AND uniacid=:uniacid  LIMIT 1', array(
                ':id'=>$roomid,
                ':uniacid'=>$_W['uniacid']
            ));
            if(!empty($live)){
    ...
    public function isFavorite($openid=null, $roomid=0) {
        ...
        if(!empty($openid) && !empty($roomid)){
            $favorite = pdo_fetch('SELECT id FROM '.tablename('ewei_shop_live_favorite'). 'WHERE uniacid=:uniacid AND roomid=:roomid AND openid=:openid AND `deleted`=0 LIMIT 1', array(
                ':uniacid'=>$_W['uniacid'],
                ':roomid'=>$roomid,
                ':openid'=>$openid,
            ));
...
//                $apiother = ihttp_get('https://api.m.panda.tv/stream/room/pull/get?roomid='.$roomid.'&roomkey='.$apiResult['data']['videoinfo']['room_key'].'&definition_option=1&hardware=1');
            ...
            case 'douyu':
                ...
                $apiResult = json_decode($apiResult['content'], true);
                //$apiResult['data']['hls_url'] = strstr($apiResult['data']['hls_url'],'?',true);
                //dump($apiResult);
                ...
                $resultArr = array(
                    'status'=>$apiResult['data']['error']==0?1:0,
            ...
            /*case 'shuidi':
                //preg_match("/.*view.html\?sn=([0-9A-Z]*)/is", $url, $matchs);
            ...
            case 'qlive':
                ...
                $resultArr = array(
                    'status'=>$apiResult['result']['cameraDetail']['canView'],
                    'poster'=>$apiResult['result']['cameraDetail']['coverFileName'],
                    'hls_url'=>empty($res['hlsUrl'])?'http://v.smartcamera.163.com/qingguo-public/'. $roomid. '/playlist.m3u8':$res['hlsUrl']
                );
                break;
            case 'ys7';
    ...
    public function getLivePrice($goods=array(), $liveid=0) {
        ...
        // 是否开启多规格
        if(empty($options)){
            if(floatval($isLiveGoods['liveprice'])>=floatval($goods['minprice'])){
                return false;
            }
            $arr = array(
                'minprice'=>$isLiveGoods['liveprice'],
                'maxprice'=>$isLiveGoods['liveprice'],
            );
        }else{
            // 开启多规格重新计算min/maxprice
            $prices = array();
    ...
    public function isLiveGoods($goodsid=0, $liveid=0) {
        ...
        $livegoods = pdo_fetch('SELECT * FROM '. tablename('ewei_shop_live_goods'). ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(
            ':goodsid'=>$goodsid,
            ':liveid'=>$liveid,
            ':uniacid'=>$_W['uniacid']
        ));
    ...
    public function getLiveOptions($goodsid=0, $liveid=0, $options=array()) {
        ...
        if(empty($options)){
            // 先查询所有商品规格
            $options = pdo_fetchall('SELECT id, marketprice FROM ' . tablename('ewei_shop_goods_option') . ' WHERE goodsid=:goodsid AND uniacid=:uniacid ORDER BY displayorder ASC', array(
                ':goodsid' => $goodsid,
                ':uniacid' => $_W['uniacid']
            ));
        }
        ...
        // 再查询参与直播的规格
        $liveOptions = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_live_goods_option') . ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(
            ':goodsid' => $goodsid,
            ':liveid' => $liveid,
            ':uniacid' => $_W['uniacid']
        ), 'optionid');
        ...
        foreach ($options as &$option){
            ...
            if(floatval($liveOption['liveprice'])>=$option['marketprice']){
                continue;
            }
        ...
        if(!empty($liveid)){
            ...
            if(!empty($goods['goodsid'])){
                ...
                foreach ($goodsid as $key=>$value){
                    $goodslist[$key] = pdo_fetch('SELECT lg.*,sg.id as id,lg.id as livegid,sg.marketprice,sg.thumb,sg.title FROM '. tablename('ewei_shop_live_goods'). ' lg LEFT JOIN '. tablename('ewei_shop_goods'). ' sg ON sg.id=lg.goodsid WHERE lg.liveid=:liveid AND lg.goodsid = :goodsid AND lg.uniacid=:uniacid', array(
                        ':liveid'=>$liveid,
                        ':goodsid'=>$value,
                        ':uniacid'=>$_W['uniacid']
                    ));
