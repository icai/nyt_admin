...
define('TM_ABONUS_PAY', 'TM_ABONUS_PAY'); //分红打款
define('TM_ABONUS_UPGRADE', 'TM_ABONUS_UPGRADE'); //升级
define('TM_ABONUS_BECOME', 'TM_ABONUS_BECOME'); //成为代理商通知
...
if (!class_exists('AbonusModel')) {
    ...
    class AbonusModel extends PluginModel
    {
        ...
        public function getSet($uniacid = 0)
        {
            ...
            $set['texts'] = array(
                'aagent' => empty($set['texts']['aagent']) ? '区域代理' : $set['texts']['aagent'],
                'center' => empty($set['texts']['center']) ? '区域代理中心' : $set['texts']['center'],
                'become' => empty($set['texts']['become']) ? '成为区域代理' : $set['texts']['become'],
                'bonus' => empty($set['texts']['bonus']) ? '分红' : $set['texts']['bonus'],
                'bonus_total' => empty($set['texts']['bonus_total']) ? '累计分红' : $set['texts']['bonus_total'],
                'bonus_lock' => empty($set['texts']['bonus_lock']) ? '待结算分红' : $set['texts']['bonus_lock'],
                'bonus_pay' => empty($set['texts']['bonus_lock']) ? '已结算分红' : $set['texts']['bonus_pay'],

                'bonus_wait' => empty($set['texts']['bonus_wait']) ? '预计分红' : $set['texts']['bonus_wait'],
                'bonus_detail' => empty($set['texts']['bonus_detail']) ? '分红明细' : $set['texts']['bonus_detail'],
                'bonus_charge' => empty($set['texts']['bonus_charge']) ? '扣除提现手续费' : $set['texts']['bonus_charge']
            );
            return $set;
        }
        ...
        function getBonus($openid = '', $params = array())
        {
            global $_W;

            if (in_array('ok', $params)) {
                $ret['ok'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('ok1', $params)) {
                $ret['ok1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('ok2', $params)) {
                $ret['ok2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('ok3', $params)) {
                $ret['ok3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)   from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }

            if (in_array('lock', $params)) {
                $ret['lock'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('lock1', $params)) {
                $ret['lock1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('lock2', $params)) {
                $ret['lock2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('lock3', $params)) {
                $ret['lock3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }


            if (in_array('total', $params)) {
                $ret['total'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('total1', $params)) {
                $ret['total1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('total2', $params)) {
                $ret['total2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('total3', $params)) {
                $ret['total3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            return $ret;
        ...
        function sendMessage($openid = '', $data = array(), $message_type = '')
        {
            ...
            if ($message_type == TM_ABONUS_PAY && empty($usernotice['abonus_pay'])) {
                ...
                if($tm['is_advanced']){
                    ...
                    //代理商升级  [昵称] [旧等级] [新等级] [旧等级分红比例] [新等级] [新等级分红比例] [时间]
                    $message = '';
                    $upgradetitle = '';
                    ...
                    } elseif ($data['aagenttype'] == 2 && empty($tm['abonus_upgrade2_close_advanced'])) {
                        $tag = 'abonus_upgrade2';
                        $message = $tm['upgrade2'];
                        $upgradetitle = empty($tm['upgradetitle2']) ? '市级代理等级升级通知' : $tm['upgradetitle2'];
                    } elseif ($data['aagenttype'] == 3 && empty($tm['abonus_upgrade3_close_advanced'])) {
                        $tag = 'abonus_upgrade3';
                        $message = $tm['upgrade3'];
                    ...
                    } elseif ($member['aagenttype'] == 2) {
                        $datas[] = array('name'=>'旧市级分红比例','value'=>$data['oldlevel']['bonus2'] . '%');
                        $datas[] = array('name'=>'旧区级分红比例','value'=>$data['oldlevel']['bonus3'] . '%');
                        $datas[] = array('name'=>'新市级分红比例','value'=>$data['newlevel']['bonus2'] . '%');
                        $datas[] = array('name'=>'新区级分红比例','value'=>$data['newlevel']['bonus3'] . '%');
                    } elseif ($member['aagenttype'] == 3) {
                        $datas[] = array('name'=>'旧区级分红比例','value'=>$data['oldlevel']['bonus3'] . '%');
                        $datas[] = array('name'=>'新区级分红比例','value'=>$data['newlevel']['bonus3'] . '%');
                    }
        ...
        function getInfo($openid, $options = null)
        {
        ...
        function upgradeLevelByOrder($openid)
        {
            ...
            if ($leveltype == 4 || $leveltype == 5) { //自购升级
                ...
                if (empty($oldlevel['id'])) {
                    $oldlevel = array(
                        'levelname' => empty($set['levelname']) ? '普通代理商' : $set['levelname'],
                        'bonus' => $set['bonus']
                    );
                }
        ...
        function upgradeLevelByAgent($openid)
        {
            ...
            //6下级总人数（代理商+非代理商）7 一级下级人数（代理商+非代理商）
            //8团队总人数（代理商）9一级团队人数（代理商）
            //10下线总人数（非代理商） 11一级下线人数（非代理商）
         ...
         * @param type $mid
         */
        function upgradeLevelByBonus($openid)
        {
            ...
            if (!empty($oldlevel['id'])) {
                if ($oldlevel['bonusmoney'] > $newlevel['bonusmoney']) {
                    return; //如果存在降级，则等级不变
                }
            }
        ...
        function getBonusData($year = 0, $month = 0, $week = 0,$openid = '')
        {
            global $_W;
            ...
            //历史记录
            $bill = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_bill') . ' where uniacid=:uniacid and `year`=:years and `month`=:months and `week`=:week limit 1',
                array(':uniacid'=>$_W['uniacid'],':years' => $year, ':months' => $month, ':week' => $week));
            ...
            if (!empty($bill) && empty($openid)) {
                return array(
                    ...
                    'bonusmoney1' => round($bill['bonusmoney1'], 2),
                    'bonusmoney_send1' => round($bill['bonusmoney_send1'], 2),

                    'bonusmoney2' => round($bill['bonusmoney2'], 2),
                    'bonusmoney_send2' => round($bill['bonusmoney_send2'], 2),

                    'bonusmoney3' => round($bill['bonusmoney3'], 2),
                    'bonusmoney_send3' => round($bill['bonusmoney_send3'], 2),


                    'aagentcount1' => $bill['aagentcount1'],
                    'aagentcount2' => $bill['aagentcount2'],
                    'aagentcount3' => $bill['aagentcount3'],
            ...
            //所有区域代理商
            $aagents = pdo_fetchall('select m.id,m.openid,m.aagentlevel,m.aagenttype,m.aagentprovinces,m.aagentcitys,m.aagentareas, l.bonus1,l.bonus2,l.bonus3 from ' . tablename('ewei_shop_member') . ' m '
                . '  left join ' . tablename('ewei_shop_abonus_level') . ' l on l.id = m.aagentlevel '
                . "  where m.uniacid=:uniacid and  m.isaagent=1 and m.aagentstatus=1 {$pcondition}", array(':uniacid' => $_W['uniacid']));
            ...
            foreach ($aagents as &$a) {
                ...
                if ($a['aagenttype'] == 1) {
                    //省级代理
                    $aagentcount1++;
                } elseif ($a['aagenttype'] == 2) {
                    //市级代理
                    $aagentcount2++;
                } elseif ($a['aagenttype'] == 3) {
                    //区级代理
                    $aagentcount3++;
                }
            ...
            foreach ($aagents as &$a) {
                ...
                $a['charge'] = 0;
                $a['chargemoney'] = 0;
                if (floatval($set['paycharge']) <= 0 || ($a['bonusmoney1'] + $a['bonusmoney2'] + $a['bonusmoney3'] >= floatval($set['paybegin']) && $a['bonusmoney'] <= floatval($set['payend']))) {
            ...
            return array(
                ...
                'bonusmoney1' => round($bonusmoney1, 2),
                'bonusmoney2' => round($bonusmoney2, 2),
                'bonusmoney3' => round($bonusmoney3, 2),
                'aagentcount1' => $aagentcount1,
                'aagentcount2' => $aagentcount2,
                'aagentcount3' => $aagentcount3,
                'starttime' => $starttime,
                'endtime' => $endtime,
                'old' => false
            );
        ...
        function getTotals()
        {
            global $_W;
            return array(
                'total0' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_abonus_bill') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])),
                'total1' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_abonus_bill') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])),
                'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_abonus_bill') . ' where uniacid=:uniacid and status=2  limit 1', array(':uniacid' => $_W['uniacid']))
            );
        }
    }
