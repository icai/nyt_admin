...
//        $parent = pdo_fetchall('select id,uniacid,agentid from '.tablename('ewei_shop_member').' where uniacid = :uniacid',array(':uniacid'=>35));
//
//        foreach ($parent as $val){
//            if ($val['agentid'] !== '0' && $val['id'] != $val['agentid']){
//                p("commission")->saveRelation($val['id'],$val['agentid'],1);
//            }
//        }
//        dump($parent);
//        die;
        ...
        $sql = "select dm.*,dm.nickname,dm.avatar,p.id as pid,p.nickname as parentname,p.avatar as parentavatar  from " . tablename('ewei_shop_member') . " dm "
            . " left join " . tablename('ewei_shop_member') . " p on p.id = dm.headsid "
            . " where dm.uniacid = " . $_W['uniacid'] . " and dm.isagent =1 and dm.isheads = 1 {$condition} ORDER BY dm.agenttime desc";
        ...
        $total = pdo_fetchcolumn("select count(dm.id) from" . tablename('ewei_shop_member') . " dm  "
            . " left join " . tablename('ewei_shop_member') . " p on p.id = dm.headsid "
            . " where dm.uniacid =" . $_W['uniacid'] . " and dm.isagent =1 and dm.isheads = 1 {$condition}", $params);
        ...
        if ($_GPC['export'] == '1') {
            ...
            m('excel')->export($list, array(
                "title" => "团队数据-" . date('Y-m-d-H-i', time()),
                "columns" => $columns
            ));
        }
...
//            $starttime = strtotime($_GPC['time']['start']);
//            $endtime = strtotime($_GPC['time']['end']);
//            $condition .= " AND agenttime >= :starttime AND dm.agenttime <= :endtime ";
//            $params[':starttime'] = $starttime;
//            $params[':endtime'] = $endtime;
//        }
        if ($_GPC['followed'] != '') {
        ...
        if ($hasheads) {
            ...
            $total = pdo_fetchcolumn("select count(dm.id) from" . tablename('ewei_shop_member') . " dm "
                ...
                . " where dm.uniacid =" . $_W['uniacid'] . " and dm.headsid = ".$headsid." {$condition}", $params);
            $sql = "select dm.*  from " . tablename('ewei_shop_member') . " dm "
                . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}"
                . " where dm.uniacid = " . $_W['uniacid'] . " and dm.headsid = ".$headsid."  {$condition}   ORDER BY dm.createtime desc";
        ...
        if ($_GPC['export'] == 1) {
            ...
            m('excel')->export($list, array(
                "title" => "团员信息-" . date('Y-m-d-H-i', time()),
                "columns" => $columns
            ));
        }
...
//                pdo_update('ewei_shop_dividend_init',array('status'=>0),array('headsid'=>$member['id'],'uniacid'=>$_W['uniacid']));
    ...
    function init(){
        ...
        foreach ($members as $member) {
            ...
            $data = pdo_fetchall("select  r.id,r.pid,m.isheads,m.headsstatus from " . tablename('ewei_shop_commission_relation') . " r "
